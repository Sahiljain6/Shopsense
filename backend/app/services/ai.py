...
class AIOrchestrator:
    ...
    def vector_context(self, message: str, products: list[Product]) -> str:
        if self.settings.pinecone_api_key:
            try:
                ...
                pass
        return "\n".join(
            f"{p.name}: {p.description} ${p.price} rating {p.rating}"
            for p in products[:5]
        )
    ...
    def answer(
        self, message: str, products: list[Product], memory: list[str] | None = None
    ) -> str:
        if not products:
        ...
        return (
            ai
            or f"I recommend {', '.join(p.name for p in products[:3])}. They best match your request by rating, stock, and catalog relevance."
        )

...
class CatalogService:
    ...
    def search(self, query: str, limit: int = 10) -> list[Product]:
        ...
        stmt = select(Product)
        if terms:
            filters = [Product.name.ilike(f"%{t}%") | Product.brand.ilike(f"%{t}%") | Product.description.ilike(f"%{t}%") for t in terms]
    ...
    def reviews(self, product_id: int) -> list[Review]:
        return list(self.db.scalars(select(Review).where(Review.product_id == product_id)))
